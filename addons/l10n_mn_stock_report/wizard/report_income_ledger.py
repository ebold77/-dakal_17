...
class ReportincomeLedger(models.TransientModel):
    ...
    def get_lines(self, data, picking_type_id):
        ...
        self._cr.execute(income_query, params)
        income_query_obj = self._cr.dictfetchall()
        where = ''
        group_by = ''
        select_type = ''
        self._cr.execute("SELECT cast(m.date as date) AS date, m.id AS move_id, m.origin_returned_move_id AS return_id, rp.id as partner_name, "
                        ...
                        "GROUP BY m.id,m.date,m.origin_returned_move_id,m.picking_id,rp.id,m.purchase_line_id, "+group_by+" "
                            "p.name, p.id ,m.product_id,p.origin,p.transit_order_id "
                        "ORDER BY m.date "
        ...
        #         if income['partner_name']:
        #             partner = self.env['res.partner'].search([('id', '=', income['partner_name'])])
        #             partner_name = partner.name
        #         # if income['cost_amount'] !=0:   
        #         #     cost_amount = income['cost_amount']
        #         # elif income['cost_amount1'] !=0:
        #         #     cost_amount = income['cost_amount1']
        ...
        #         #       """
        #         #     params = income['picking_id'], income['sale_id']
        ...
        #         vals = {
        #             'date_done': income['date'].strftime('%Y-%m-%d'),
        #             'name': income['name'],
        #             'origin': income['origin'],
        #             'partner_name': partner_name,
        #             'qty': income['product_qty'],
        #             'picking_id': income['picking_id'],
        #             # 'sale_id': income['sale_id'],
        #             # 'transit_order_id': income['transit_order_id'],
        #             'list_price': listprice_ammount,
        #             'discount': discount,
        #             'sale_price': price_total,
        #             'amount_tax': price_tax,
        #             'subtotal': price_subtotal,
        #             'cost_amount': income['cost'],
        #         }
        #         lines.append(vals)
        ...
        for warehouse_id in wiz['warehouse_ids']:
            ...
            for val in get_line.values():
                ...
                for v in val['lines'].values():   
                    ...
                    self._cr.execute("SELECT stock_landed_cost_id FROM stock_landed_cost_stock_picking_rel WHERE stock_picking_id = %s" %v['picking_id'])
                    lcost_ids = self._cr.dictfetchall()
                    lcost_amount = 0 
                    if lcost_ids:
                        for lcost_id in lcost_ids:
                            lcost = self.env['stock.landed.cost'].search([('id','=', lcost_id['stock_landed_cost_id'])])
                            lcost_amount += lcost.amount_total
                    ...
                    sheet.write(row, 6, v['cost'] + lcost_amount, font_size_8)
                    total_qty += v['qty']
                    total_cost +=v['cost']
                    row +=1
